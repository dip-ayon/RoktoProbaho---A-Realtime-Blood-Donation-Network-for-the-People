...
import type { UserProfile, BloodRequest, DonationOpportunity, BloodType, UrgencyLevel, DonationOffer } from '@/lib/types';
...
const initialRequests: BloodRequest[] = [
    { 
        ...
        offers: [
            { id: 'offer_1', donorId: 'd_1', donorName: 'Dip Kundu', donorBloodType: 'A+', donorAvatarUrl: 'https://placehold.co/128x128.png', status: 'Completed' }
        ] 
    },
    { 
        id: 'req_2', 
        ...
        offers: [
            { id: 'offer_2', donorId: 'd_2', donorName: 'Anika Tabassum', donorBloodType: 'B-', donorAvatarUrl: 'https://images.unsplash.com/photo-1535643302794-19c3804b874b?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3NDE5ODJ8MHwxfHNlYXJjaHw3fHxwZXJzb24lMjBwb3J0cmFpdHxlbnwwfHx8fDE3NTI1MjkwNTR8MA&ixlib=rb-4.1.0&q=80&w=1080', status: 'Pending' },
            { id: 'offer_3', donorId: 'd_3', donorName: 'Raihan Chowdhury', donorBloodType: 'O+', donorAvatarUrl: 'https://images.unsplash.com/photo-1600180758890-6b94519a8ba6?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3NDE5ODJ8MHwxfHNlYXJjaHwxfHxwZXJzb24lMjBwb3J0cmFpdHxlbnwwfHx8fDE3NTI1MjkwNTR8MA&ixlib=rb-4.1.0&q=80&w=1080', status: 'Pending' },
        ] 
    },
     { id: 'req_3', requesterName: 'Self', bloodType: 'O-', quantity: 1, location: 'Banani, Dhaka', urgency: 'Medium', date: '2024-06-18', status: 'Open', neededByDate: '2024-06-20', offers: [] },
];
            ...
            geocoder.geocode({ location: newMarker }, (results, status) => {
                if (status === 'OK') {
                    if (results && results[0]) {
                        ...
                        toast({ title: 'Location not found', description: 'Could not find an address for the selected point.', variant: 'destructive' });
                    }
                } else {
                    toast({ title: 'Geocoder failed', description: `Could not determine address: ${status}`, variant: 'destructive' });
                }
            });
        }
    }, [toast]);
  ...
  return (
        <form onSubmit={handleSubmit} className="grid gap-4 p-1 pr-4">
            <div className="grid gap-2">
                ...
                <Input id="patient-name" placeholder={t('requestBlood.patientNamePlaceholder')} required value={patientName} onChange={e => setPatientName(e.target.value)} />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="grid gap-2">
                    ...
                    <Input id="quantity" type="number" placeholder={t('requestBlood.quantityPlaceholder')} required min="1" value={quantity} onChange={e => setQuantity(parseInt(e.target.value, 10) || 1)} />
                </div>
            </div>
              ...
              <div className="grid gap-2">
                ...
                <Popover>
                    <PopoverTrigger asChild>
                        <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !neededDate && "text-muted-foreground")}>
                            <Icons.calendar className="mr-2 h-4 w-4" />
                            {neededDate ? format(neededDate, "PPP") : <span>{t('requestBlood.pickDate')}</span>}
                        </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0">
                        <Calendar mode="single" selected={neededDate} onSelect={setNeededDate} initialFocus disabled={(date) => date < new Date(new Date().setDate(new Date().getDate() - 1))} />
                    </PopoverContent>
                </Popover>
              </div>
            </div>
...
export default function Dashboard() {
            ...
            return currentRequests.map(req => {
                if (req.id === requestId) {
                     ...
                     toast({
                        ...
                        description: `You have ${accepted ? 'accepted' : 'declined'} the offer from ${offerToUpdate.donorName}.`,
            ...
            return currentRequests.map(req => {
                if (req.id === requestId) {
                    ...
                    toast({
                        ...
                        description: `Thank you for confirming. ${offerToComplete.donorName}'s donation count has been increased.`,
                    });
  ...
  return (
    <div className="container mx-auto py-10 px-4">
      <div className="mb-4">
        <h1 className="text-3xl font-bold font-headline mb-2">{t('dashboard.welcome', { name: user.name })}</h1>
        <p className="text-muted-foreground">{t('dashboard.modeIntro', { mode: t(`dashboard.${mode}`) })}</p>
      </div>
                            ...
                            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                                <div>
                                    <div className="flex items-center gap-4">
                                        ...
                                        <Badge variant={req.status === 'Open' ? 'destructive' : req.status === 'Completed' ? 'default' : 'secondary'}>{req.status}</Badge>
                                    </div>
                        ...
                        {(req.offers && req.offers.length > 0) && (
                            <div className="p-4 border-t">
                                <h4 className="font-semibold mb-3 text-sm">
                                    Offers Received ({req.offers?.filter(o => o.status === 'Pending').length || 0} Pending)
                                </h4>
                                            ...
                                            <div className="flex items-center gap-2 flex-wrap justify-start sm:justify-end w-full sm:w-auto">
                                                {offer.status === 'Pending' && req.status === 'Open' && (
                                                    <>
                                                        ...
                                                        <Button size="sm" variant="outline" onClick={() => handleOfferResponse(req.id, offer.id, false)}>Decline</Button>
                                                    </>
                                                )}
                                                {offer.status === 'Accepted' && (
                                                    <>
                                                        ...
                                                        <Button size="sm" variant="outline"><Icons.phone className="mr-2 h-4 w-4" /> Contact</Button>
                                                        <Button size="sm" variant="secondary" onClick={() => handleBloodCollected(req.id, offer.id)}>
