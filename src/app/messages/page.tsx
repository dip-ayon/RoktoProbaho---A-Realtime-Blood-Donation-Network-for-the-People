...
export default function MessagesPage() {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    // Access avatarUrl without changing your global User type
    const userAvatar: string = (user as any)?.avatarUrl ?? '';
    ...
    setTimeout(() => {
      ...
      const reply: ChatMessage = {
        ...
        senderName: selectedConversation.partnerName,
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
      const repliedConversation: Conversation = {
        ...updatedConversation,
  ...
  const ChatView = () => (
    <div className="flex flex-col h-full">
      <header className="flex items-center gap-4 p-4 border-b">
        ...
        <Avatar>
          <AvatarImage src={selectedConversation?.partnerAvatar || undefined} data-ai-hint="person portrait" />
          <AvatarFallback>
          ...
          >
            {msg.sender === 'other' && (
              <Avatar className="h-8 w-8">
                <AvatarImage src={msg.avatar || undefined} data-ai-hint="person portrait" />
      ...
      <ScrollArea className="flex-1">
        <CardContent className="p-0">
          {conversations.map((convo) => (
            ...
            >
              <Avatar>
                <AvatarImage src={convo.partnerAvatar || undefined} data-ai-hint="person portrait" />
                <AvatarFallback>{convo.partnerName?.charAt(0)?.toUpperCase() ?? 'U'}</AvatarFallback>
              </Avatar>
              ...
              <div className="flex-grow overflow-hidden">
                ...
                <div className="flex justify-between items-start">
                  ...
                  {convo.unreadCount > 0 && (
                    <Badge className="w-5 h-5 p-0 flex items-center justify-center">
                      {convo.unreadCount}
                    </Badge>
                  )}
                </div>
