    ...
    useEffect(() => {
        if (user) {
            ...
            if (user.lastDonationDate) {
                setLastDonation(new Date(user.lastDonationDate));
            }
        }
    }, [user]);
            ...
            <Card>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="grid gap-2">
                        ...
                        <Popover>
                            ...
                            <PopoverContent className="w-auto p-0">
                                <Calendar mode="single" selected={dob} onSelect={setDob} initialFocus captionLayout="dropdown-buttons" fromYear={1950} toYear={2010}/>
                            </PopoverContent>
                        </Popover>
                    </div>
                     <div className="grid gap-2">
                    ...
                    <div className="grid gap-2">
                        ...
                        <Popover>
                            <PopoverTrigger asChild>
                                <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !lastDonation && "text-muted-foreground")}>
                                    <Icons.calendar className="mr-2 h-4 w-4" />
                    ...
                    <div className="grid gap-2 md:col-span-2">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Button variant="outline"><Icons.upload className="mr-2 h-4 w-4"/>{t('settings.profileDetails.upload')}</Button>
                        </div>
                    ...
                    >
                        {(['Available', 'Temporarily Unavailable', 'Not Available'] as AvailabilityStatus[]).map(status => (
                            <div key={status} className="flex items-center space-x-2">
                                ...
                                <Label htmlFor={status.replace(/ /g, '-')} className="font-normal">{t(`settings.profileDetails.${status.replace(/ /g, '').toLowerCase()}` as any)}</Label>
                            </div>
                        ))}
                    </RadioGroup>
                </div>
